...
if(!isset($_POST['submit'])) { //if one try to access this file direct
    ...
    if(!preg_match("/^[a-zA-Z'. -]+$/", $campaignName)) {    
        ...
        // check if there is already an event name with same inputted campaignName by the organizer
        ...
        if($resultCheck > 0) {
            ...
            $campaignImagePath = "D:/xampp/htdocs/helphands/assets/images/campaignImages/".$filename;
            ...
            move_uploaded_file($tempname,$campaignImagePath);
             ...
             // if the event input data successfully inserted in database then redirect him to success page 
             if($insertSuccess) {
